...
configure_git() {
  ...
  if [ "$STRAP_ADMIN" -gt 0 ]; then
    # git config --global commands may require access to
    # global .gitconfig files owned by admin accounts
...
if [ "$STRAP_SUDO" -eq 1 ]; then
  ...
  sudo mkdir -p "$HOMEBREW_PREFIX/"{Caskroom,Cellar,Frameworks}
  sudo chmod 775 "$HOMEBREW_PREFIX/"{Caskroom,Cellar,Frameworks}
  sudo chown -R "$user" "$HOMEBREW_PREFIX"
  logk
  # Prevent "Permission denied" errors on Git directories
  log "TODO: Prevent Permission denied errors on default \$GIT_DIR"
  if [ "$MACOS" -gt 0 ]; then
